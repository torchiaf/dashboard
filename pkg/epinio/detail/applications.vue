...
export default Vue.extend<Data, any, any, any>({
  ...
  methods: {
    ...
    async fetchCommits() {
      const {
        usernameOrOrg, repo, branch, commit
      } = this.value.appSource.git;
      ...
      if (branch?.name) {
        ...
        this.gitDeployment.commits = await this.$store.dispatch(`${ this.gitType }/fetchRevisionCommit`, {
          username: usernameOrOrg, repo, revision: commit
        });
      }
    },
    setCommitDetails() {
