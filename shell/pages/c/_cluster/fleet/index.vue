...
import { FLEET, POD, MANAGEMENT, SCHEMA } from '@shell/config/types';
  ...
  methods: {
    ...
    async onResourceRowClick(value) {
      if (value?.row?.detailLocation) {
        ...
        const route = this.$router.resolve(value.row.detailLocation);

        console.log(value?.row?.detailLocation, route);

        this.iframeSrc = route.path;
